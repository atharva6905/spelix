...
- **SafetyScore** (FR-SCOR-01) — user-facing label "Movement Quality Score". Sagittal-observable inputs: torso lean (squat/DL), knee angle at depth (squat), elbow angle range (bench), shoulder angle range (bench), ankle dorsiflexion + heel rise (squat, planned via sagittal heel-y baseline). Score < 3.0 triggers mandatory top-of-page warning. Knee valgus, true wrist-over-elbow frontal alignment, and true scapular retraction require a frontal-plane camera and are deferred (ADR-AUDIT-2026-05-22).
- **TechniqueScore** (FR-SCOR-02) — sagittal-observable inputs: depth angle, torso lean, hip/knee angles, shoulder angle at bottom, elbow angle at bottom (bench). Elbow flare, grip width, stance width require a frontal-plane camera and are deferred (ADR-AUDIT-2026-05-22).
- **PathBalanceScore** (FR-SCOR-03) — bar path anterior-posterior (A-P) deviation (`ap_deviation_px`), path consistency, bar path classification (bench). Nuckols bar-over-midfoot anchor. Lateral weight shift requires a frontal-plane camera and is deferred (ADR-AUDIT-2026-05-22).
...
The 16 sagittal-view metrics that Sessions 4–7 populate live in `backend/app/cv/sagittal_metrics_registry.py` as a `frozenset[SagittalMetricEntry]`. Key names are FINAL — renaming is a breaking change requiring a JSONB-key migration. To add a new sagittal metric in Sessions 4–7: (a) write the extractor in `metric_extraction.py`, (b) flip `computed_yet=True` on the existing registry entry (do NOT add new entries — the 16 are fixed), (c) add an integration test asserting the key shows up in `rep_metrics.metrics`. Auto-flow scoring metrics (Session 4: `depth_classification`, `ecc_con_ratio`) also flip `in_scoring=True` and require a new branch in `scoring.py` + threshold config entry. The expert portal renders "Not yet computed" for every entry where `computed_yet=False`; no panel changes needed per-metric. Threshold flagging for these metrics flows through the existing FR-EXPV-08 path with `section='unvalidated_metrics'`; `ThresholdFlagService.create_flag` bypasses the v1-config current-value lookup for that section (returns `current_value=0.0`, `current_citation=None`). The DB-level CHECK constraint on `threshold_flags.section` (migration `7c4af3e51f08`) enumerates all five allowed sections and blocks typos at the data layer. Companion key `heel_rise_flag` is written alongside `ankle_dorsiflexion_deg` but is NOT a separate registry row — the description on the ankle entry names it.

### Side-agnostic landmark access (Session 2, ADR-LIFTER-SIDE-DETECTION)
Landmark indices are no longer hardcoded in `metric_extraction.py` or `signal_processing.py`. Every entry point that touches MediaPipe landmark indices accepts a `lifter_side: Literal["left","right"]` parameter (default `"right"` for backward compatibility) and resolves indices via `landmark_indices_for_side(side)` from `app/cv/lifter_side.py`. The pipeline computes the side once per analysis between Step 3 (quality-gate pass) and Step 4 (angle timeseries) via `detect_lifter_side(landmarks_session, fps)`, persists it on `analyses.lifter_side`, and threads it through `compute_angle_timeseries` + `extract_rep_metrics`. Subject's right (even indices 12/14/16/24/26/28/30/32) is the fallback on tied or ambiguous detections — it matches the pre-refactor hardcoded default, so right-side fixtures see zero behavioural change. `barbell_detection.py`'s wrist-midpoint averaging is intentionally side-agnostic (bilateral) and stays unchanged. When extending `AnalysisDetail` or any Analysis mock factory, set `obj.lifter_side = None` explicitly — the MagicMock + Pydantic gotcha applies.
