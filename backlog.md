...
| D-035 | **Pose extraction too slow for full 1080p@59fps within 900s task timeout on 2-vCPU droplet.** Surfaced by D-034 verification on 2026-04-16: with D-034 memory fix in place, `atharva-bench-no-weight.mov` (1352 frames @ 1080p) runs for the full 15-min `process_analysis` timeout without OOM, but MediaPipe BlazePose Heavy doesn't finish pose extraction before the task is cancelled. Memory stays at 639 MB throughout. CPU pinned at 180-196% (100% per core). Fix paths: (a) downscale frames to 720p before pose extraction (3-4x faster, same accuracy within clinical grade), (b) switch to BlazePose Lite for 1080p clips, (c) raise `process_analysis` timeout further (but 15 min is already high for UX), (d) upgrade droplet to higher-vCPU tier. Preferred: (a) — matches the annotation 720p cap already in place. 720p pipeline fully works end-to-end per 2026-04-16 verification. **Session 35.5 update**: PR #61 shipped (a) 720p pose cap but bench measurements showed BlazePose Heavy is ~150ms/frame regardless of resolution — fix (a) didn't move the needle. Pivoted to telemetry-first approach (ADR-058): `analyses.timing_json` + VIDEO mode + 1800s timeout + 60s/120s upload cap shipped together so production tells us where the budget actually goes. Tier 2 (ffmpeg fps-normalize OR D-036 GPU) decided after 1 week of timing_json data per ADR-059. | M | — | NFR-OPER-02 | partial |
