...
## Post-L2 beta ops — /ship-loop run 13 (2026-06-15, `sl-513b`) — #303 retry-gate SHIPPED + REVERTED (prod incident); → #311

Redesigned #303 as **Option 1 (retry-the-migrate, then gate)**: 5×10s migrate retry + `MIGRATED` flag, rollback only on a *persistent* failure (fixes the #305 readiness race). Full `/design` → `/implement` → T2 chain (spec/quality/security all PASS) → /code-review (1 cosmetic nit, fixed: trailing `sleep 10` → `[ "$m" = 5 ] || sleep 10`) → human approval → merged **PR #309** (`83e2a0a`). **On its own post-merge deploy the gate worked CORRECTLY** — it caught a genuinely-failing `alembic upgrade head` (5 attempts, all failed) and rolled back cleanly to `93abe1c` (a TRUE rollback, unlike #305; prod healthy, no outage). **Root cause exposed:** the migrate command `docker compose exec -T backend uv run alembic upgrade head` has never run successfully in prod — `uv run` (no `--no-dev`) hits `Permission denied` syncing dev deps into the non-writable `--no-dev` venv, and even `uv run --no-dev` hits `No 'script_location' key found` (alembic config not found in the exec cwd). OBS-1's errexit-off had masked this on every prior deploy. **Reverted #309 via PR #310 (`fb747f1`)** to restore deployability — revert's own deploy ran green (droplet `fb747f1`, containers healthy, `/health`=200). **#303 CLOSED (not_planned, superseded by [#311](https://github.com/atharva6905/spelix/issues/311))** which owns the migrate-command fix + re-introducing the gate. PR #309 docs-hygiene commit `4613926` preserved the spec+quality reviewer memory.

**LESSON:** ADR-DEPLOY-01's real-deploy requirement earned its keep — the live deploy caught a latent migrate-command bug that inspection + CI + the full 3-gate review chain all passed. A correctly-built rollback gate is only as useful as the command it gates; verify the gated command actually *succeeds* on a real deploy, not just that the gate logic is sound.

## Post-L2 beta ops — /ship-loop run 14 (2026-06-15, `sl-4d31`) — #311 migrate-command fix SHIPPED + VALIDATED LIVE ✅

Fixed the two-defect prod migrate command surfaced by run 13. Full `/design` (Option A: bake migrations into the image, not bind-mount) → `/implement` (spelix-tdd) → T2 chain (spec/quality/security all PASS) → /code-review (1 LOW finding fixed: assert rollback ends in `exit 1`/fail-red; 1 accepted) → human approval → merged **PR #313** (`b4377889`). Three changes: (1) `backend/Dockerfile` — `COPY alembic.ini ./` + `COPY alembic/ ./alembic/` after `COPY app/ ./app/` (they were never in the image → "No script_location"); (2) `.github/workflows/ci.yml` — migrate line → `uv run --no-dev alembic -c /app/alembic.ini upgrade head` (bare `uv run` dev-synced into the read-only `--no-dev` venv → Permission denied), wrapped in the #303 5×retry `MIGRATED`-gate; (3) `backend/tests/unit/test_deploy_workflow.py` — string-assertion regression guards. **VALIDATED ON REAL DEPLOY (ADR-DEPLOY-01):** PR #313's own Deploy to Production ran GREEN (3m21s, run `27549061372`) — droplet `alembic current` = `9fffb59ba45f (head)` (the corrected command ran cleanly, no perm/config error — first successful CI migrate ever), backend+worker+redis `(healthy)`, `/health`=200. **#311 CLOSED (completed).** Reviewer memory (spec/quality/security) + this backlog entry preserved as docs hygiene.

**LESSON:** the migrate command that "worked" for the life of the project had in fact NEVER run in CI — OBS-1's errexit-off swallowed it on every deploy, and schema stayed current only via out-of-band manual applies. The #303 gate (run 13) was the instrument that finally surfaced it. Bake deploy-time artifacts (migrations) into the image rather than relying on bind-mounts/host-checkout sync — the image should be self-describing.
...
| #300 | Pin `appleboy/ssh-action` from mutable `@v1` to immutable commit SHA `@0ff4204d…  # v1.2.5` (the secret-bearing deploy step) — closes the supply-chain vector flagged by the #275 security review. SHA independently verified (`v1`==`v1.2.5`==`0ff4204d`) → zero behavior change. Scope held: first-party `actions/*` + `astral-sh/setup-uv@v4` left mutable (lower blast radius) → noted, not churned. Full T2 chain all PASS; `/code-review` 0 findings; human "Approve & merge". Deploy green, droplet `e59816a` healthy. PR #301. | done | XS | T2→T2 | `e59816a` | `.github/workflows/ci.yml` |
| #299 | Bound the deploy primary build in `timeout 1320` (22m) so the failure-path rollback can't be starved by the shared 30m `command_timeout`; migrate now runs ONLY on build success (also fixes a latent bug: `alembic upgrade` ran after a broken build). **DESIGNED first** (`/design` → spec+plan local in `docs/internal/`, approved). Quality insight: `appleboy/ssh-action` runs WITHOUT `set -e`, so the `if` captures build status + converts the old `command_timeout` session-kill into an in-session rollback. Full T2 chain all PASS; `/code-review` 0 findings; human "Approve & merge". **Live deploy-script rewrite — its own Deploy to Production ran the new happy-path (build→migrate→health→exit 0) green; droplet `8526e83`, containers recreated + healthy.** PR #302. Filed #303 for the pre-existing OBS-1 (failing `alembic upgrade` doesn't trigger rollback). | done | S | T2→T2 | `8526e83` | `.github/workflows/ci.yml` |
...
| #286 | Wire engine into harness skills: ship-loop `--auto`/`--watch` self-feeding (per-task step-0 `claim` + heartbeat, `release` as first cleanup action, session id `sl-<4hex>` + `CLAIMS_WORKTREE`; explicit-queue mode also claims+skips live-owned); groom new sweep 6 (Claims: `board` digest + `reclaim-stale` + `gc-labels`, never claims, report-only-safe); new read-only `claims` status skill. Implementer mandated `superpowers:writing-skills`. Verified against live repo: `ready` → 8 eligible (excludes needs-human/needs-design), `board` → [], smoke exit 0. Governance-reviewer PASS (diff-only; cleared the autonomy-vs-merge-gate adversarial trap — claim layer is issue-selection upstream of the unchanged step-7 approval gate) → **T0 self-merge**. CI green (Deploy=skipping). PR #288. | done | S | T0→T0 | `49c897f` | `.claude/skills/ship-loop/SKILL.md`, `.claude/skills/groom/SKILL.md`, `.claude/skills/claims/SKILL.md` |
...
| #216 | FR-BRAIN-16 real-Postgres integration test for the consent-cascade tombstone predicate (origin: PR #213 /code-review MEDIUM — unit test only string-asserts rendered SQL; doesn't prove the #203 row-selection bug). New `backend/tests/integration/test_coach_brain_consent_cascade.py`: row-selection matrix (seed/active-empty-unconfirmed/active-empty-confirmed/active-nonempty/deprecated) asserts ONLY active+empty+unconfirmed is tombstoned (`deprecated` + `rejected_reason='source_consent_withdrawn'`) + end-to-end array_remove→tombstone cascade. **Shared-DB safety:** `soft_delete_empty_unconfirmed()` is table-wide + caller-commits, so the sweep runs UNCOMMITTED and is `rollback()`'d (never persists); only uniquely-tagged inserted rows are committed + DELETE-cleaned. Hardened in `99c8ebe` after I flagged the commit-the-sweep hazard. 2 tests pass locally vs real PG; auto-skips in CI (no DATABASE_URL). Governance-reviewer PASS → **T0 self-merge**. Unit test retained per ADR-BRAIN-12. PR #280. | done | S | T0→T0 | `70dd0ff` | `backend/tests/integration/test_coach_brain_consent_cascade.py` |
| #236 | Expert upload page hygiene (4 batched /code-review items from PR #233). (1) shared `FieldError` component (`text-sm text-red-600` + `role="alert"` — closes a11y gap on the fileError site) replacing 4 inline sites across ExpertPaperUploadPage + UploadPage; (2) hoisted `fillForm`/`resetApiMocks` test helpers; (3) shared `clearErrors()` used by BOTH submit-start and `resetForm()` (fixes the hand-rolled reset that omitted `setDoiError(null)`); (4) completing-phase 409 discard hint "Your uploaded file was discarded; submitting again will re-upload it." (failing-phase tracked via local `let` — `uploadPhase` state stale in async closure). Also broadened backend `error.message` surfacing (security-verified non-leaking: `expertFetch` throws a plain object so `err.message` only sees hardcoded literals; expert-endpoint messages are curated). Tier T1→**T2** (user-facing strings; never-down-tier). Security PASS; spec/quality PASS-WITH-NITS — all nits fixed in-loop (`7279e53` shared clearErrors + over-build test; `d1890f7` non-vacuous Upload-Another reset test + upload-phase-409 negative test). 840 frontend tests pass, tsc clean. Human-approved at T2 gate (per-file diff + verbatim security verdict). PR #281. | done | S | T1→T2 | `7fafe84` | `frontend/src/components/FieldError.tsx`, `frontend/src/pages/ExpertPaperUploadPage.tsx`, `frontend/src/pages/UploadPage.tsx`, test files |
| #235 | Typed `ExpertApiError extends Error` (status/code?/message/detail?) thrown from `expertFetch` via `buildExpertApiError` (unwraps FastAPI `{detail:{error}}`, plain detail obj, Pydantic array→synthesized msg, string detail, no-detail→`Request failed (HTTP n).` fallback; never throws from the error path) + exported `isExpertApiError` guard (instanceof + name/status duck-type). `ExpertPaperUploadPage` catch migrated to the guard, surfaces `err.message` for INVALID_FILENAME/INVALID_PDF/QUEUE_UNAVAILABLE/INVALID_STATE, + "Try again" control (phase→idle, preserves form values+file). Cross-file migration (beta/admin/profiles/analyses) DEFERRED w/ doc comment → follow-up **#283**. Tier T1→**T2** (user-facing strings). spec/quality/security PASS (SaMD PASS verbatim). **`/code-review` caught a CRITICAL the spec/quality/security chain missed**: the shared `expertFetch` throw-shape change regressed the un-migrated sibling consumer `ExpertPortalPage.handleApprovePaper` (still read `apiErr.error?.code` → DUPLICATE_DOI/FR-EXPV-06/#260 fell to generic copy); fixed `d1b7c93` (migrate to guard + non-vacuous regression test throwing the real shape; independent sweep cleared all other `@/api/expert` consumers). 848 frontend tests pass, tsc clean; CI green; droplet `ac96dc6` healthy; light prod E2E PASS (`/expert/papers/upload`+`/expert`→`/login` guard, 0 console errors on correct routes). Human-approved at T2 gate. PR #282. | done | M | T1→T2 | `ac96dc6` | `frontend/src/api/expert.ts`, `frontend/src/pages/ExpertPaperUploadPage.tsx`, `frontend/src/pages/ExpertPortalPage.tsx`, test files |
...
| ID | Title | Status | Size | Tier (prov→actual) | Commit | Files |
|----|-------|--------|------|--------------------|--------|-------|
| #231 | FR-EXPV-02 ownership guard on `complete_paper_upload` — 403 `NOT_PAPER_OWNER` after get_by_id/404, BEFORE all destructive paths (INVALID_PDF + DUPLICATE_DOI-race cleanup), when `(extra_metadata or {}).get("uploaded_by") != str(user["id"])` unless admin; legacy missing-`uploaded_by` rows fail-closed. Closes the security reviewer's standing re-flag from PR #227. Spec/quality (1 MEDIUM deferred: extract `_is_paper_owner`)/security PASS (T2 chain); 1 CI red = integration-fixture bug (per-request `uuid4()` override → upload/complete as different experts; guard worked as designed), fixed test-only `ac6f4f2`. PR #277. | done | S | T2→T2 | `896440e` | `backend/app/api/v1/expert.py`, 3 test files |
| #267 | Ingestion crash on `quality_tier=None` (upload schema optional, `ChunkPayload.quality_tier` non-optional → ValidationError on approval; pre-existing, widened by #234's DOI-less types). Fix option 2: ingestion-time floor `metadata.quality_tier or "L4_guideline"` in `IngestionService._build_payloads` + `DocumentMetadata.quality_tier` widened to `\| None` — also heals the 8 NULL-tier prod rows with no backfill (`L5_expert_opinion` from the issue does NOT exist in QualityTier; correctly not invented). Spec PASS; /code-review: 0 findings (single ChunkPayload write site confirmed). PR #276. | done | S | T2→T1 | `cd4f6b9` | `backend/app/services/ingestion.py`, `backend/tests/unit/test_ingestion.py` |
...
| #234 | FR-EXPV-02 DOI required iff `document_type == "research_paper"` — `RagDocumentUploadRequest.doi` → `str \| None` + model_validator (422 with guidance copy); `request_paper_upload` normalize/dedup gated on non-null DOI; new Document Type select on upload page (was hardcoded — DOI-less types unreachable), conditional marker/submit-gate/payload omission. Spec/quality/security PASS (T2 chain); /code-review → follow-up #267 (pre-existing quality_tier ingestion crash); dedup-bypass-via-misclassification accepted (single-partner scale). PR #266. Prod E2E PASS (select live w/ 5 options; DOI `*`↔`(optional)` toggles both directions; 0 console errors). | done | S | T2→T2 | `2c71ea5` | `backend/app/schemas/rag_document.py`, `backend/app/api/v1/expert.py`, `frontend/src/api/expert.ts`, `frontend/src/pages/ExpertPaperUploadPage.tsx`, 3 test files |
| #232 | FR-RAGK-05/FR-EXPV-05 shared `DoiLink` component (anchor + `target/rel` in one place, standardized `data-testid="doi-empty"` gray-dash fallback, className/children/aria-label props) replacing 3 hand-rolled sites (CitationTooltip, AdminPage, ExpertPortalPage); em-dash tests now testid-scoped. Spec PASS (13/13); /code-review: 0 blocking, 2 optional notes (plain-text DOI sites, href encoding nit). PR #265. | done | XS | T1→T1 | `404088a` | `frontend/src/components/DoiLink.tsx` (+tests), CitationTooltip, AdminPage, ExpertPortalPage, 2 test files |
...
| ID | Title | Status | Size | Tier (prov→actual) | Commit | Files |
|----|-------|--------|------|--------------------|--------|-------|
| — | Run-4 follow-up: agent-memory preservation codified in ship-loop step 8 (first cleanup bullet — copy `.claude/agent-memory/**` writes from the task worktree to the main checkout before ANY ExitWorktree; agents inherit session cwd, removal destroys their MEMORY.md updates). Grep gate + writing-skills pressure test PASS. T0 self-merge: governance-reviewer PASS, CI green. PR #255. | done | XS | T0→T0 | `8418fcc` | `.claude/skills/ship-loop/SKILL.md` |
| #248 | Discipline nesting — `verification-before-completion` into /implement Step 5 (fresh command evidence after final commit, no "should pass") + /ship-loop step 5 claims discipline ("CI green"/"deployed" only from actual output); `receiving-code-review` into the Step 3 fix loop (findings are claims, verify before changing, evidence-backed refutations round-trip to the reviewer, no performative agreement); `writing-skills` dispatch guard for tasks touching `.claude/skills|agents/**`. 3 commits (one per task), grep gates all green. T0 self-merge: governance-reviewer PASS, CI green. PR #253. | done | S | T0→T0 | `4d82413` | `.claude/skills/implement/SKILL.md`, `.claude/skills/ship-loop/SKILL.md` |
| #247 | /bugfix rebuild — body rewritten around `superpowers:systematic-debugging` as REQUIRED SUB-SKILL (4-phase root-cause protocol binding, no "just try this"); overrides: Phase-1 evidence commands (pytest w/ pose-extraction exclusion, vitest), Phase-4 fix is TDD, 3-strike stop → /implement Step 4 blocker report, commit-at-green; `context: fork` preserved; report table + Evidence column. Grep gates 1/1 + pressure test PASS (NameError → no immediate edit). T0 self-merge: governance-reviewer PASS, CI green. PR #252. | done | XS | T0→T0 | `3828379` | `.claude/skills/bugfix/SKILL.md` |
| #246 | TDD nesting — `superpowers:test-driven-development` as REQUIRED SUB-SKILL in all three implementer agents (spelix-tdd/cv-engineer/ai-engineer): Skill tool granted (`tools: …, Skill`), `## TDD Protocol (nested sub-skill)` block with 4 Spelix overrides (gates before RED, fixed test commands, 3-strike stop, commit-at-green); existing protocols kept as override detail. Grep gates 3/3 + pressure test PASS (first action = Skill invocation). T0 self-merge: governance-reviewer PASS, CI green. PR #250. | done | S | T0→T0 | `5700f9f` | `.claude/agents/spelix-{tdd,cv-engineer,ai-engineer}.md` |
...
| #237 | Worktree-safe hooks — new `_lib.js` (`resolveRoot` via git-common-dir parent, `inLinkedWorktree`), stop-gate skip + post-edit-lint pyright skip in linked worktrees, pre-compact/session-end/session-start handoff root-anchoring (+ worktree stamps), graphify-hint root check, settings.json hook commands → `node "$CLAUDE_PROJECT_DIR/.claude/hooks/<x>.js"`, smoke-test worktree-mode cases (TDD red-first; quality FAIL→fix: assertions anchored to `resolveRoot()`, was leaking writes into real handoff.md). Spec/quality/security PASS. Merged on recorded in-session human approval (gate). PR #242. Pending: fresh-session `$CLAUDE_PROJECT_DIR` interpolation verification (fallback documented in #237). | done | M | T2→T2 | `f37bf3e` | `.claude/hooks/*` (8 files incl. new `_lib.js`), `.claude/settings.json` |
| #239 | In-session approval gate — ship-loop step 7 T1+ terminal `needs-human` → APPROVAL GATE (gist + AskUserQuestion; recorded PR comment BEFORE merge; T2 gist must embed verbatim security verdict + per-file diff = explicit human diff review; T3 ultra-before-gate; headless//groom fallback to needs-human); step 8 branches for all five outcomes (closed = sole sanctioned `discard_changes: true`, human-gated); governance.md T1/T2 amended; ADR-HARNESS-02 (+bidirectional cross-link to ADR-HARNESS-01). Spec FAIL→fix→PASS, quality FAIL→fix→PASS (outcome-enum drift), security PASS (control-weakening analysis: every non-approve path fail-safe). Merged on recorded in-session human approval. PR #244. | done | M | T2→T2 | `e1e614f` | `.claude/skills/ship-loop/SKILL.md`, `.claude/rules/governance.md`, `decisions.md` |
...
## Completed — cv-audit Session 6: Bar-Coordinate Math (2026-05-23, session 68)

Session 6 of the cv-audit effort. 2 sagittal-view metrics consuming bar-coordinate math, plus 2 reusable phase-frame helpers. Both metrics compute-only (`in_scoring=False`). #4 `bar_to_hip_distance` (deadlift) is a JSONB dict at four phase frames (setup / liftoff / knee_pass / lockout); #14 `shoulder_protraction_proxy_px` (bench) is a per-rep float. Phase-frame identification via new public helpers `identify_liftoff_frame` and `identify_knee_pass_frame`. Bar trajectory uses the wrist-midpoint proxy (landmarks 15+16 mean) — matches `compute_bar_path_from_landmarks` fallback, so no pipeline reorder. `RepMetrics.metrics` value type widened to `dict[str, RepMetricValue]` (`RepMetricValue = float | str | dict[str, float | None]`) so the phase-frame map coexists with float metrics (ADR-022 follow-up). Side-agnostic via `_facing_sign`. Plan expansion: PR #163 merged `c6d6cde`. Implementation: PR #164 merged `cc30829`.

| ID | Title | Status | Size | Deps | SRS IDs | Commit | Files |
|----|-------|--------|------|------|---------|--------|-------|
| L2-SAGITTAL-BAR-01 | #4 bar_to_hip_distance phase-frame dict (deadlift, compute-only) + identify_liftoff_frame/identify_knee_pass_frame helpers + RepMetrics.metrics dict-value widening + registry flip + side-agnosticism mirror test | done | M | L2-SAGITTAL-INFRA-01..04 | FR-EXPV-03 | `cc30829` | `backend/app/cv/metric_extraction.py`, `backend/app/cv/sagittal_metrics_registry.py`, `backend/tests/unit/test_metric_extraction_sagittal.py`, `backend/tests/unit/test_metric_extraction.py` |
| L2-SAGITTAL-BAR-02 | #14 shoulder_protraction_proxy_px (bench, compute-only) + facing-sign + registry flip + side-agnosticism mirror test | done | S | L2-SAGITTAL-INFRA-01..04 | FR-EXPV-03 | `cc30829` | `backend/app/cv/metric_extraction.py`, `backend/app/cv/sagittal_metrics_registry.py`, `backend/tests/unit/test_metric_extraction_sagittal.py` |

**Integration test**: `backend/tests/integration/test_pipeline_sagittal_metrics.py::test_session6_*` runs the full pipeline on the deadlift + bench fixtures. Deadlift: 5 reps, `bar_to_hip_distance` dict populated per rep with ≥2/4 phase frames resolved (rep 0 resolved all four: setup=0.81, liftoff=0.79, knee_pass=0.69, lockout=0.16). Bench: 13 reps, `shoulder_protraction_proxy_px` finite float per rep (range -0.19..+0.06). Sanity bounds wide (±5.0) — expert validates the meaningful sub-range post-onboarding via FR-EXPV-08. 27 new unit tests (helpers + extractors + analyzer integration + side-agnosticism mirror). No migrations (additive JSONB keys). No threshold config entries. No frontend changes (registry-driven panel auto-renders).

## Completed — cv-audit Session 7: Complex Multi-Frame Analysis (2026-05-23) — EFFORT COMPLETE

Final session of the cv-audit effort. 3 sagittal metrics needing cross-rep context / shape classification, all compute-only. Preceded by a mandatory `/plan` spike + expanded TDD plan (committed `797c35f`). #2 `lumbar_flexion_proxy_delta_deg` uses a standing-baseline frame (squat: global `reps[0].start_frame`; DL: prev-rep lockout, first-rep `liftoff−1`) and an explicit `_proxy` suffix + naming honesty. #6 `bar_path_classification` is a side-agnostic v0 J-curve heuristic on the wrist-midpoint x-trajectory. #16 `technique_consistency_std` is population std (ddof=0) of depth_angle (squat) / lockout_torso_lean_deg (DL), injected via a post-pass. `RepMetricValue` widened to allow `None` (cannot-compute = JSON null, never 0.0). A post-merge calibration finding (lumbar −165° from deep-squat occlusion) was remediated by a `dy≤0` guard (PR #168). All 16 sagittal metrics now `computed_yet=True`. ADR-LUMBAR-FLEXION-PROXY-NAMING. Impl PR #167 (`f93d1ee`); occlusion-guard PR #168 (`75f6d0d`).

| ID | Title | Status | Size | Deps | SRS IDs | Commit | Files |
|----|-------|--------|------|------|---------|--------|-------|
| L2-SAGITTAL-COMPLEX-01 | #2 lumbar_flexion_proxy_delta_deg (squat+DL, compute-only) + identify_standing_baseline_frame + _lumbar_proxy_angle + dy≤0 occlusion guard + facing-sign + registry flip + naming honesty + side-agnosticism mirror test | done | L | L2-SAGITTAL-INFRA-01..04 | FR-EXPV-03 | `75f6d0d` | `backend/app/cv/metric_extraction.py`, `backend/app/cv/sagittal_metrics_registry.py`, `backend/tests/unit/test_metric_extraction_sagittal.py` |
| L2-SAGITTAL-COMPLEX-02 | #6 bar_path_classification (bench, compute-only, side-agnostic v0 heuristic) + _classify_bar_path + session_modal helper + registry flip + mirror test | done | M | L2-SAGITTAL-INFRA-01..04 | FR-EXPV-03 | `f93d1ee` | `backend/app/cv/metric_extraction.py`, `backend/app/cv/sagittal_metrics_registry.py`, `backend/tests/unit/test_metric_extraction_sagittal.py` |
| L2-SAGITTAL-COMPLEX-03 | #16 technique_consistency_std (squat+DL, compute-only, population std post-pass) + _inject_technique_consistency_std + RepMetricValue None widening + registry flip | done | M | L2-SAGITTAL-INFRA-01..04 | FR-EXPV-03 | `f93d1ee` | `backend/app/cv/metric_extraction.py`, `backend/app/cv/sagittal_metrics_registry.py`, `backend/tests/unit/test_metric_extraction.py` |

**Integration test**: `test_pipeline_sagittal_metrics.py::test_session7_*` runs the full pipeline on all 3 atharva fixtures. Squat (left, 6 reps): lumbar_delta=6.31° on the one clean rep, None on occluded reps (post-guard); technique_consistency_std=29.58°. Bench: bar_path_classification labels populated. Deadlift: lumbar_delta + technique_consistency_std populated. All 3 green. No migrations, no threshold config, no frontend changes (registry-driven panel auto-renders). **cv-audit effort COMPLETE — all 7 sessions merged.**
