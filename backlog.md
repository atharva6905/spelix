...
| D-014 | Droplet OOM mitigation — add 2GB swap, deploy PR #27, verify E2E | M | — | **CRITICAL.** 2GB droplet cannot run MediaPipe Heavy + backend simultaneously. 3 consecutive sessions (20–22) blocked by this. After DO dashboard reboot: `fallocate -l 2G /swapfile && chmod 600 /swapfile && mkswap /swapfile && swapon /swapfile && echo '/swapfile none swap sw 0 0' >> /etc/fstab`, then `git pull && docker compose build backend worker && up -d`. |
