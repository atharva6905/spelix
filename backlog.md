...
| D-040 | Replace fixed-threshold rep detection state machine with `scipy.signal.find_peaks` peak/valley extraction in `backend/app/cv/rep_detection.py::detect_reps`. Tuning knobs become signal-relative (`prominence_deg`, `min_distance_s`) rather than per-exercise absolute thresholds (160°/90°, 150°/110°, 160°/70°). Unlocks partial-lockout / partial-depth reps (bodyweight bench, fatigued lifts, parallel-only squats) that today fail silently with 0 reps. Audit + update `test_rep_detection.py` fixtures that encoded the old state-machine behavior. See ADR-REPDET-01. | M | — | FR-CVPL-15, FR-REPM-01, FR-REPM-05 | open — **session 45 priority 1** |
| D-041 | Fix degenerate-input path in `backend/app/cv/scoring.py` ScoreComponent implementations: when `rep_metrics` is empty OR confidence category is "Very Low", return `None` / "Not available" for per-dimension scores instead of defaulting to 10.0. Observed on P3-007 E2E analysis (`cea2312b-…`): 0 reps + Confidence "Very Low / Unable to score reliably" but UI rendered Technique 10.0 and Control 10.0 — trust-violating contradiction. FormScoreCards already handles `null` → "Not available" rendering, so frontend no-op. See ADR-REPDET-01 Consequences. | S | — | FR-SCOR-02, FR-SCOR-04, FR-SCOR-07 | open — **session 45 priority 1 (bundle with D-040)** |
