...
### Backend
- **Create:** `backend/alembic/versions/010_add_timing_json.py` — Alembic migration adding nullable JSONB column `analyses.timing_json`.
...
**Files:**
- Create: `backend/alembic/versions/010_add_timing_json.py`
...
```
Expected: `009_papers_bucket_rls (head)`. If not, STOP and investigate.

- [ ] **Step 2: Create the migration file**

Create `backend/alembic/versions/010_add_timing_json.py` with this exact content:
...
```
Expected output ends with `Running upgrade 009_papers_bucket_rls -> 010_add_timing_json`. Then verify head:
```bash
...
```bash
git add backend/alembic/versions/010_add_timing_json.py backend/app/models/analysis.py
...
- New Alembic migration `010_add_timing_json` adds nullable JSONB column.
