...
**Consequences**: Idea → spec → plan → issues → tier-reviewed execution is one continuous pipeline with the human at design approvals and T1+ merges. Reviewer knowledge compounds via agent memory (tier precedents, recurring findings). Two execution paths never diverge — /design files issues, never code. Supersedes nothing; extends the v2 ADRs. Design spec: local-only `docs/internal/2026-06-09-harness-v3-design.md`.
