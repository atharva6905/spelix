...
> ADRs are grouped by domain below; see the Decision Index for the full list.
> Append-only: superseded ADRs stay in place with a status note and a cross-link to the superseding ID.

## Decision Index

| ID | Title | Domain | Status |
|----|-------|--------|--------|
| ADR-001 | Greenfield Build | Foundational / Platform | Accepted |
| ADR-002 | ARQ over RQ | Foundational / Platform | Accepted |
| ADR-006 | Python 3.12 Mandatory | Foundational / Platform | Accepted |
| ADR-007 | Node.js 22 LTS Mandatory | Foundational / Platform | Accepted |
| ADR-008 | JSONB Over JSON | Foundational / Platform | Accepted |
| ADR-011 | Status Column — VARCHAR(30) with CHECK | Foundational / Platform | Accepted |
| ADR-013 | opencv-python-headless in Docker | Foundational / Platform | Accepted |
| ADR-014 | GSD Framework Coexistence | Foundational / Platform | Accepted |
| ADR-026 | Worktree is Vite + React SPA, Not Next.js | Foundational / Platform | Accepted |
| ADR-040 | analysis_expert_reviews Table Name (Session 26) | Foundational / Platform | Accepted |
...
| ADR-DEPLOY-01 | Deploy-script control-flow changes require real-deploy validation (inspection + CI can't catch post-`up -d` timing) | Foundational / Platform | Accepted |
...
| ADR-017 | GPT-4o Vision Fallback Threshold = 0.7 | CV & Scoring | Accepted |
| ADR-018 | ThresholdConfig — Versioned JSON in Repo, PR Review as Approval | CV & Scoring | Accepted |
| ADR-022 | Per-Rep Metrics Dict — Widened to `dict[str, float \| str]` | CV & Scoring | Accepted |
| ADR-023 | Detection Result Stored as JSONB on Analyses Row | CV & Scoring | Accepted |
| ADR-025 | PDF Bar Path Chart — Matplotlib Lazy Import | CV & Scoring | Accepted |
| ADR-029 | MediaPipe Tasks API, Not Legacy `solutions` (Session 13) | CV & Scoring | Accepted |
| ADR-034 | Aspect-Ratio-Aware Framing Gate Threshold (Session 20) | CV & Scoring | Accepted |
| ADR-035 | Status Endpoint Includes quality_gate_result for Rejection Guidance (Session 20) | CV & Scoring | Accepted |
| ADR-044 | Squat Rep Detection Threshold Adjustment (Session 26) | CV & Scoring | Accepted |
| ADR-053 | Framing quality gate samples peak bbox across whole clip, not first 5 frames ... | CV & Scoring | Accepted |
| ADR-054 | Framing + single-person gates — separate occlusion/orientation bug from tempo... | CV & Scoring | Accepted |
| ADR-056 | Analysis pipeline memory budget on 4 GB droplet — post-MediaPipe OOM (Session... | CV & Scoring | Accepted |
| ADR-057 | Streaming barbell tracking eliminates 8.4 GB peak allocation (Session 34) | CV & Scoring | Accepted |
| ADR-058 | D-035 instrumentation tier — measure first, optimize second (Session 35.5) | CV & Scoring | Accepted |
| ADR-059 | Telemetry-first principle for CV pipeline tuning (Session 35.5) | CV & Scoring | Accepted |
| ADR-060 | Downscale frames to 480p before HoughCircles to close D-035 (Session 39) | CV & Scoring | Superseded by ADR-STREAQ-TIMEOUT-01 |
| ADR-REPDET-01 | Replace fixed-threshold rep detection state machine with peak/valley extracti... | CV & Scoring | Superseded by ADR-REPDET-02 |
| ADR-REPDET-02 | Hybrid state-machine + peak/valley rep detection supersedes pure peak/valley ... | CV & Scoring | Accepted |
| ADR-SCOR-DEGENERATE-01 | Degenerate scoring guard lives at pipeline level, not inside `ScoreComponent`... | CV & Scoring | Accepted |
| ADR-REPDET-03 | D-044 atharva-bench.mov over-count — defer fix post-L2 after investigation re... | CV & Scoring | Accepted |
| ADR-REPDET-04 | Rep-detection knobs flow through ThresholdConfig (Session 53, D-042) | CV & Scoring | Accepted |
| ADR-QGATE-COMMERCIAL-GYM | Anchor-based single_person gate + visibility-gated bbox framing gate | CV & Scoring | Superseded by ADR-QGATE-COMMERCIAL-GYM-CALIBRATION-01 |
| ADR-QGATE-COMMERCIAL-GYM-CALIBRATION-01 | `_FRAMING_MIN_FRACTION` 0.20 → 0.18 follow-up | CV & Scoring | Accepted |
| ADR-AUDIT-2026-05-22 | Sagittal-view scope and deferred multi-camera roadmap | CV & Scoring | Accepted |
| ADR-LIFTER-SIDE-DETECTION | Visibility-weighted lifter-side detection with anchor robustness (Session 2) | CV & Scoring | Accepted |
| ADR-SAGITTAL-METRICS-REGISTRY | Frozenset as single source of truth for the 16 sagittal-view metrics (Session 3) | CV & Scoring | Accepted |
| ADR-AUTO-FLOW-REFINEMENTS | Two refinement metrics bypass the compute-only-until-validated rule (Session 4) | CV & Scoring | Accepted |
| ADR-LUMBAR-FLEXION-PROXY-NAMING | Naming honesty + baseline/heuristic/consistency choices for the 3 complex met... | CV & Scoring | Refined by ADR-DEPTHFRAME-DROPOUT-GATE |
| ADR-DEPTHFRAME-DROPOUT-GATE | Dropout-aware depth-frame selection + auxiliary-metric plausibility None-guar... | CV & Scoring | Refined by ADR-CV-OBLIQUE-3D-DIRECTION |
| ADR-BENCH-BARPATH-NONE-INTERIM | Bench bar-path is visibility-gated to None until a real bar-tracker exists (2... | CV & Scoring | Accepted |
| ADR-DEADLIFT-FIRSTREP-BASELINE | Deadlift first-rep lumbar baseline anchors to its own lockout, not the pre-li... | CV & Scoring | Accepted |
| ADR-ANGLE-SERIES-VALIDITY-GATE | Validity-gate + linear-interpolate + clamp the angle time-series upstream of ... | CV & Scoring | Accepted |
| ADR-R5-CONFIDENCE-INTERP-SURFACING | Surface per-rep confidence + R2 interpolation fraction in the expert portal, ... | CV & Scoring | Accepted |
| ADR-CV-RUNNING-MODE-NO-CHANGE | Keep MediaPipe RunningMode.VIDEO — IMAGE mode is worse for deep-squat occlusi... | CV & Scoring | Accepted |
| ADR-CV-OBLIQUE-3D-DIRECTION | Squat-bottom loss is barbell-plate occlusion; the fix is front-oblique captur... | CV & Scoring | Accepted |
| ADR-P2-001 | Qdrant Cloud Free Tier for Phase 2 (Session 14) | RAG & Retrieval | Accepted |
| ADR-RAG-01 | Cohere Rerank 4.0 as Cross-Collection Score Normaliser (Session 14) | RAG & Retrieval | Accepted |
| ADR-RAG-02 | Docling for PDF Parsing (Session 14) | RAG & Retrieval | Accepted |
| ADR-RAG-03 | Matryoshka 1024 Dimensions for Both Qdrant Collections (Session 14) | RAG & Retrieval | Accepted |
| ADR-RAG-04 | LLM-as-Judge Faithfulness Instead of HHEM T5 (Session 16) | RAG & Retrieval | Accepted |
...
| ADR-BRAIN-06 | Postgres JSONB for Per-Athlete Episodic Memory (Phase 4 Schema Reservation) | Coach Brain & Distillation | Accepted |
| ADR-BRAIN-07 | Distillation Pipeline as Standalone StateGraph in Phase 3 (Session 14) | Coach Brain & Distillation | Accepted |
| ADR-DISTILL-01 | Candidate storage in a new `coach_brain_candidates` table (Session 41) | Coach Brain & Distillation | Accepted |
| ADR-DISTILL-02 | Invocation via streaq task, not `asyncio.create_task` (Session 41) | Coach Brain & Distillation | Accepted |
| ADR-DISTILL-03 | Slim `BrainCoveService` for single-claim verification (Session 41) | Coach Brain & Distillation | Accepted |
| ADR-DISTILL-04 | `Chunk` added alongside `ChunkPayload` in `app/schemas/rag.py` (Session 41) | Coach Brain & Distillation | Accepted |
| ADR-DISTILL-05 | Never persist raw `str(exc)` to admin-visible DB columns (Session 41) | Coach Brain & Distillation | Accepted |
| ADR-BRAIN-08 | Seed Coach Brain entries are retrieval-eligible via cold-start fallback (Sess... | Coach Brain & Distillation | Accepted |
| ADR-PHASE2-EVAL-FALLBACK | Quality-score gates fall back to `faithfulness` until Phase 4 ships `overall`... | Coach Brain & Distillation | Accepted |
| ADR-BRAIN-REVIEW-01 | Near-atomic approve — Postgres INSERT + Qdrant upsert in one request (Session... | Coach Brain & Distillation | Accepted |
| ADR-DISTILL-06 | BrainCoveService Haiku 4.5 max_tokens — 512 question / 2048 answer (Session 46) | Coach Brain & Distillation | Accepted |
| ADR-BRAIN-09 | Agent retrieval queries need per-exercise vocabulary enrichment for Cohere Re... | Coach Brain & Distillation | Accepted |
| ADR-DISTILL-07 | lifecycle_decision Qdrant API migration from search() to query_points() (Sess... | Coach Brain & Distillation | Accepted |
...
| ADR-021 | Phase 1 Coaching — Stream-Then-Reparse Pattern (Tech Debt) | Coaching & CoVe | Accepted |
| ADR-024 | Worker OpenAI Client — Single Instance, Graceful Degradation | Coaching & CoVe | Accepted |
| ADR-P2-022a | Non-Streaming Chat for Phase 2 MVP (Session 18) | Coaching & CoVe | Accepted |
| ADR-P2-022b | SafetyFilter.apply_text() for Plain String Filtering (Session 18) | Coaching & CoVe | Accepted |
| ADR-COVE-01 | CoveVerificationService Haiku 4.5 / Sonnet 4.6 max_tokens budgets (Session 48) | Coaching & CoVe | Accepted |
| ADR-COVE-02 | CoVe claim extraction — principle-level, not measurement-level (Session 49) | Coaching & CoVe | Accepted |
| ADR-COVE-03 | CoVe claim extraction — inversion-guard + extrapolation-guard (Session 50) | Coaching & CoVe | Accepted |
| ADR-COVE-RERUN-01 | CoVe re-run on admin content edit is best-effort | Coaching & CoVe | Accepted |
| ADR-CHAT-01 | `ChatService.send_message` uses `get_by_id_with_relations` (Session 59) | Coaching & CoVe | Accepted |
| ADR-003 | Supabase FK — No DDL Constraint | Data & Privacy | Accepted |
| ADR-004 | 7-Day Artifact Retention | Data & Privacy | Accepted |
| ADR-009 | "Movement Quality" Not "Safety Score" | Data & Privacy | Accepted |
| ADR-BRAIN-05 | GDPR Article 9 Explicit Consent for All Health Data Processing (Session 14) | Data & Privacy | Accepted |
| ADR-041 | Expert Reviewer Role System — Dual Access (Session 26) | Data & Privacy | Accepted |
| ADR-042 | Signed Read URLs for Private Storage Artifacts (Session 26) | Data & Privacy | Accepted |
| ADR-051 | PostHog in Cookieless / Memory-Persistence Mode to Avoid GDPR Cookie Banner (... | Data & Privacy | Accepted |
| ADR-052 | BetaRequest.email 201-Response PII Removal + UNIQUE Index on Model (Session 29) | Data & Privacy | Accepted |
...
| ADR-030 | Frontend Upload — REST PUT, Not TUS Resumable (Session 13) | Infra & Ops | Accepted |
| ADR-031 | Status Table — Operational `→ failed` Edges (Session 13) | Infra & Ops | Accepted |
| ADR-BRAIN-04 | Defer ARQ → streaq Migration to Phase 3 (Session 14) | Infra & Ops | Superseded by ADR-BRAIN-04-reversal |
| ADR-037 | Uvicorn Proxy Headers + FastAPI redirect_slashes=False (Session 24) | Infra & Ops | Accepted |
| ADR-038 | Docker Privilege Escalation for Root SSH Access (Session 24) | Infra & Ops | Accepted |
| ADR-043 | H.264 Re-encoding for Browser Video Playback (Session 26) | Infra & Ops | Accepted |
| ADR-045 | Host-Mounted Resource Directories Use CWD-Based Path Resolution (Session 27) | Infra & Ops | Accepted |
| ADR-047 | Supabase Realtime Requires Both Publication Membership and REPLICA IDENTITY F... | Infra & Ops | Accepted |
| ADR-048 | Droplet Sizing for L2 Private Beta — Basic 2 vCPU / 4 GB (Session 28) | Infra & Ops | Accepted |
| ADR-055 | streaq `process_analysis` task timeout raised to 900s (restore ADR-BRAIN-04 P... | Infra & Ops | Superseded by ADR-STREAQ-TIMEOUT-01 |
| ADR-BRAIN-04-reversal | streaq migration pulled into L2 sprint (Day 3-9) | Infra & Ops | Accepted |
...
| ADR-REASONING-SIDEBAR-01 | P3-007 "How AI Reasoned" sidebar design choices | Frontend | Accepted |
| ADR-EXPV-08 | Threshold validation UI uses a flag-only DB table; PR review remains the appr... | Frontend | Accepted |
| ADR-EXPERT-03 | Expose annotated video on expert analysis detail page (Session 68) | Frontend | Accepted |
| ADR-032 | Tests Must Exercise Real Factories with Source-Patched Third-Party Modules (S... | Process & Tooling | Accepted |
| ADR-036 | Langfuse Observability — Two-Flag Singleton, Best-Effort Tracing, eval_scores... | Process & Tooling | Accepted |
| ADR-050 | Manual Beta-Request Approval Flow Through Sprint BETA — No Auto-Gate (Session... | Process & Tooling | Accepted |
| ADR-TIMELINE-01 | Phase 3 pulled forward into the May 3 L2 sprint | Process & Tooling | Accepted |
| ADR-ROOTCAUSE-01 | Fix at the source, not the symptom (Session 59) | Process & Tooling | Accepted |
| ADR-E2E-01 | Per-role E2E test accounts via Supabase service-role script | Process & Tooling | Accepted |
...
## ADR-HARNESS-01: Harness v3 — nested superpowers chains, /implement review layer, memory reviewers
**Context**: Harness v2 (ADR-GOVERNANCE-01, ADR-AUTONOMY-01, ADR-HOOKS-01) automated execution (issue → gated merge) but had no ideation/planning front half, dispatched specialists with no per-task review, and carried four stale pre-v2 skills (/plan, /parallel, /team, /phase-gate, /escalate). Agent memory dirs existed but stayed empty — dispatch prompts ("report findings only") suppressed writes.
**Decision**: (1) `/design` — ideation router that NESTS the superpowers plugin skills (`superpowers:brainstorming` → `superpowers:writing-plans`) under a Spelix override preamble (specs/plans local-only in `docs/internal/`, FR-ID + SaMD discipline, AskUserQuestion for option picks), terminating in groomed GitHub issues — issues are the interface; ship-loop stays the only execution engine. (2) `/implement` — execution layer between harnesses and specialists: preflight + worktree + routing table (cv/migration/ai/tdd) + TIER-SCALED review chain (T0/XS: self-review; T1: +spec-reviewer; T2/T3: spec → quality → security), same-instance fix loops, 3-strike escalation. (3) Named read-only reviewer agents with `memory: project` (spelix-spec-reviewer, spelix-quality-reviewer, spelix-governance-reviewer — the latter extracts the anonymous T0 fresh-context reviewer; governance.md amended) plus consult/update memory bookends made REQUIRED in all memory agents and dispatch templates. (4) coaching/rag/langgraph engineers consolidated into `spelix-ai-engineer` (opus). (5) /plan, /parallel, /team, /phase-gate, /escalate retired (preflight salvaged into /implement, escalation format into /implement step 4).
**Consequences**: Idea → spec → plan → issues → tier-reviewed execution is one continuous pipeline with the human at design approvals and T1+ merges. Reviewer knowledge compounds via agent memory (tier precedents, recurring findings). Two execution paths never diverge — /design files issues, never code. Supersedes nothing; extends the v2 ADRs. Design spec: local-only `docs/internal/2026-06-09-harness-v3-design.md`. Superseded in part by ADR-HARNESS-02 (T1/T2 merge terminal behavior, 2026-06-10).

## ADR-HARNESS-02: In-session approval gate supersedes terminal needs-human for T1/T2 merges
...
## ADR-ANGLE-SERIES-VALIDITY-GATE: Validity-gate + linear-interpolate + clamp the angle time-series upstream of rep detection (2026-05-24, cv-audit R2)

**Context.** The same post-Session-7 investigation (`docs/superpowers/investigations/2026-05-23-cv-occlusion-rootcause.md`) showed that MediaPipe `RunningMode.VIDEO` (D-035/ADR-058) emits zero-filled dropout frames near the hardest phase of squat/bench (13–46 % of frames per rep). `calculate_angle` on an all-origin frame returns **0°**, injecting a spurious spike into the raw angle series; `smooth_signal` (Savitzky-Golay) then over/undershoots near the spike, producing the physically-impossible `hip_angle` range the investigation observed (**−32.5°…192.5°**). That garbage reached three consumers: (1) `detect_reps` (a fake-deep valley distorts the state machine and the peak/valley fallback), (2) the **un-gated plain `argmin`** depth-frame selection in `pipeline.py` Step-7 confidence scoring, and (3) metric depth-frame selection. **R1 (ADR-DEPTHFRAME-DROPOUT-GATE)** fixed only (3) — and listed this exact upstream gate as its deferred "Alternatives considered" item. R2 is that gate: clean the series at the source so (1) and (2) are de-noised too. The user chose **Option A** (linear-interpolate all gaps + clamp) over cap-and-hold / cap-and-flag variants.

**Decision.** `compute_angle_timeseries` (`signal_processing.py`) now gates each per-joint series before smoothing:

1. **Validity gate (squat + deadlift only).** A frame's angle for a joint is trusted only when **all** of that joint's defining landmarks clear `_MIN_VIS = 0.30` (raw MediaPipe visibility, column 3) — the Tier-2 "minimum landmark" principle (FR-CVPL-21), the same threshold as `metric_extraction._S5_MIN_VIS`/`_vis_ok` (kept in sync; not imported, to avoid a circular import). Joint→landmark dependencies live in `_JOINT_LANDMARK_DEPS` (hip_angle→{shoulder,hip,knee}, knee_angle→{hip,knee,ankle}). Invalid frames are set to **NaN** rather than letting the 0° spike enter. **Bench is deliberately excluded from the gate** — see point 5.
2. **Linear interpolation (`_interpolate_invalid`).** Interior NaN runs are filled by `np.interp` (straight line between nearest valid frames); leading/trailing NaN runs hold the first/last valid value. Linear interpolation is **deliberately conservative**: between two valid endpoints it can only *under*-read a peak/valley, never fabricate a deeper-than-observed extremum — so it removes dropout spikes without inventing motion that wasn't seen. This satisfies the investigation's "don't fabricate long occlusions" caveat (a straight line is the conservative failure mode, not a fabricated bottom), which is why **no gap-length cap** is enforced for *interpolation*.
3. **Clamp to [0,180].** The smoothed series is `np.clip`-ed to `[0,180]` to kill any residual Savitzky-Golay over/undershoot.
4. **Fully-occluded clip.** If a joint has no valid frame anywhere, the series is left all-NaN and smoothing is skipped (the quality gate rejects such clips upstream). Length is always preserved (one value per frame).
5. **Bench excluded from the gate.** On supine bench the wrists are *systematically* near-invisible (`atharva-bench.mov`: wrist visibility median **0.008**, `<_MIN_VIS` on **~100 %** of frames). The first implementation gated `elbow_angle` on `{shoulder, elbow, wrist}` and the **sagittal integration suite caught the regression**: it NaN'd the entire bench primary series, cutting detected reps **13 → 3** and pushing `bar_touch_height_pct` to the spurious **76.4** (the exact marginal-wrist artifact ADR-DEPTHFRAME-DROPOUT-GATE flagged). Bench is therefore **not** in `_JOINT_LANDMARK_DEPS` (an absent exercise is computed/smoothed/clamped but un-gated). Verified on cached bench landmarks: excluding bench restores **13 reps** and `bar_touch_height_pct` range **(−15.39, 0.0)**. This mirrors R1 excluding bench from its depth-frame mask — bench wrist/bar-path robustness is R3/R3b (barbell detection), not R2.

**Consequences.**
- (+) Rep detection and the confidence-scoring `argmin` no longer see garbage — R2 de-noises at the source, complementing R1's metric-path mask (defense-in-depth, not redundancy). Real-fixture proof on `atharva-squat.mov` (164/1192 = 13.8 % raw dropout frames): cleaned `hip_angle` is now **[25.63°, 180.00°]** with zero values outside `[0,180]` (was −32.5°…192.5°), and the minimum cleaned value at *any* raw-dropout frame is **52.58°** — the ~0° spikes are gone, replaced by phase-consistent interpolated angles.
- (+) Backward-compatible: clean clips are unchanged (interpolation is a no-op with no NaN; clamp is a no-op for in-range values), so the deadlift path and all clean reps are untouched.
- (−) A rep whose *true* bottom sits entirely inside a long dropout run reads slightly **shallow** (a straight line under-reads the unseen extremum). This is the conservative direction and is already backstopped for metrics by R1's depth-frame mask. Surfacing *which* reps were heavily interpolated is deferred to **R5** (expert-portal confidence) — a documented future cap (≈ `round(0.15·fps)` frames) would flag, not drop, such reps; R2 itself never drops a rep.
- (−) `_MIN_VIS` is duplicated (0.30) rather than imported from `metric_extraction` to avoid a circular import; the cross-reference comment keeps them in sync.

**Alternatives considered.**
- **Cap + hold for long gaps.** Rejected — holding a shallow value through a long squat-bottom occlusion can make the state machine miss a genuinely-deep rep; *dropping a real rep is worse than under-reading its depth*. Evidence also shows dropout bursts are short (valid frames 3–6 away from each mis-selected bottom), so the long-gap branch rarely fires.
- **Cap + flag rep low-confidence.** Rejected for R2 scope — surfacing per-rep confidence/"why" is the separate R5 follow-up; bundling it expands into confidence plumbing.
- **Do it in `detect_reps` instead of `compute_angle_timeseries`.** Rejected — gating once at the series source fixes all three consumers; gating in `detect_reps` would leave the confidence `argmin` and metric path still reading the un-gated series.
- **Leave R2 unbuilt (R1 is enough).** Rejected — R1 protects only the metric depth-frame; rep detection and the Step-7 `argmin` still ingested garbage.

**Related.** Implements the deferred R2 from **ADR-DEPTHFRAME-DROPOUT-GATE** (R1) — same investigation. ADR-058 (VIDEO running mode) is the upstream source (R4 may revisit it). Files `backend/app/cv/signal_processing.py` (`compute_angle_timeseries`, new `_landmarks_visible`/`_interpolate_invalid`, `_MIN_VIS`/`_JOINT_LANDMARK_DEPS`), `backend/tests/unit/test_signal_processing.py` (7 R2 tests). Backlog row `L2-CV-DEPTHFRAME-R2`. R3b/R4/R5 remain open per the investigation doc.

## ADR-R5-CONFIDENCE-INTERP-SURFACING: Surface per-rep confidence + R2 interpolation fraction in the expert portal, reusing existing signals over a per-metric None-reason taxonomy (2026-05-24, cv-audit R5)

**Context.** R1/R2/R3/R6 made the CV pipeline return `None` *honestly* when a metric can't be computed (dropout, occlusion, plausibility guards). But the expert portal's Unvalidated Metrics panel rendered a bare "Not yet computed" for **both** unimplemented metrics (`computed_yet=false`) **and** genuine cannot-computes (`computed_yet=true`, value `null` this rep) — an expert validating thresholds (FR-EXPV-08) could not tell a true cannot-compute from a bug, nor *why* it happened. R5 surfaces the why. The design fork: emit an explicit per-metric reason at each of the 40+ `None` sites (a reason taxonomy), versus reuse the per-rep landmark-confidence already computed + add a single new "how much did R2 reconstruct" signal. The user chose the latter (Option B).

**Decision.**
1. **Reuse the existing per-rep Tier-5 confidence.** `RepMetric.confidence_score` is already computed (FR-CVPL-24) and already serialized by the expert API. The panel now renders it as a **categorical** label (`getConfidenceCategory` → High/Moderate/Low/Very Low, FR-CVPL-25 — never the raw decimal) in a per-rep column-header chip and as the reason line on a cannot-compute cell. No new confidence computation.
2. **Add ONE precise signal computed in a single place — the interpolation fraction.** New pure fn `compute_invalid_frame_mask` (`signal_processing.py`) reuses R2's exact gating predicate (`_JOINT_LANDMARK_DEPS` + `_landmarks_visible` + `_MIN_VIS`) to produce a clip-length bool mask of R2-gated frames. The pipeline (Step 4) computes it once and stores per-rep `mean(mask[start:end+1])` on a new nullable `RepMetric.interpolation_fraction` column (migration `47c8e446162e`). This is the R5 hook **ADR-ANGLE-SERIES-VALIDITY-GATE** deferred — "% of this rep R2 reconstructed." Bench → all-False → 0.0 (not R2-gated), matching R2's bench exclusion.
3. **Three-state cell in the panel.** `computed_yet=false` → "Not yet computed" (metric unimplemented); `computed_yet && value!=null` → value + Flag (unchanged); `computed_yet && value==null` → "Cannot compute" + confidence category + "{N}% interpolated" (suppressed when 0).
4. **No 40-site refactor; no new endpoint; no Pydantic response-schema change** (`ExpertAnalysisDetail.rep_metrics` is `list[dict[str, Any]]`).

**Consequences.**
- (+) An expert sees, per rep, the landmark-confidence label and how heavily R2 reconstructed the signal — `None` now shows *why* without touching every extractor. Real-fixture proof: `atharva-squat.mov` per-rep interpolation fractions `[0.31, 0.08, 0.24, 0.62, 0.04, 0.22]` (rep 4 = 62% reconstructed — the deep-squat occlusion); `atharva-bench.mov` all `0.0`.
- (+) The single computation site shares R2's predicate exactly (drift-guarded by a `_MIN_VIS`-boundary test), so the "% reconstructed" can never disagree with what R2 actually gated.
- (−) The reason is **coarse** (rep-level confidence + interpolation fraction), not per-metric — an expert sees "Very Low confidence · 62% interpolated," not "hip landmark <0.30 at the bottom." A per-metric taxonomy was rejected as scope creep (40+ sites, regression risk) for marginal expert value.
- (−) `interpolation_fraction` is meaningful only for R2-gated exercises (squat/DL); bench is always 0.0 (documented), NULL = pre-R5 analysis.

**Alternatives considered.**
- **Per-metric None-reason taxonomy (instrument all 40+ `None` sites).** Rejected — large refactor across the whole metric-extraction surface with real regression risk, for a more precise reason than experts need at this stage.
- **Confidence label only (frontend-only, no backend).** Rejected — honest but generic; it would not surface R2's reconstruction, the specific signal this investigation produced.
...
**Related:** the L2-sprint Day 1-2 Track B plan; FR-EXPV-02
...
**Fallback (stop-loss trigger per the internal sprint plan):** if migration blocks Phase 3 start by >5 days, revert to ARQ with `max_jobs=1`, `job_timeout=900` and migrate later. Revert commit kept at the branch's base SHA.
...
**Consequences.** Build-time supply-chain integrity now matches BlazePose: a tampered or corrupted weight download fails the build loudly instead of shipping. The manifest is coupled to the docling version pinned in `uv.lock` — a docling bump that changes model revisions will (by design) break the build until the manifest is regenerated from a trusted run. That coupling is the intended forcing function, documented in the Dockerfile comment. Regenerate via `cd /app/models/docling && find . -type f -not -path '*/.cache/*' | sort | xargs sha256sum > docling_models.sha256` against a trusted, freshly-built image. The check adds negligible build time and zero runtime cost. Scope limitation: `sha256sum -c` detects substitution or corruption of the 27 listed artifacts but does NOT detect the *addition* of extra unlisted files an attacker might drop into the model dir; this is acceptable for the threat model because docling loads only the manifested artifact set, so an unreferenced extra file is never executed. The manifest's LF line endings are pinned via `backend/.gitattributes` (`docling_models.sha256 text eol=lf`) so a Windows `core.autocrlf=true` re-commit cannot silently introduce a trailing `\r` that would break `sha256sum -c` in the Linux build; a byte-level regression test (`test_manifest_has_no_carriage_returns`) backstops the pin.
...
**Context**: The L2-sprint plan set a hard gate for Landing V1 live on prod by end of Day 2 of the 19-day sprint (2026-04-15) while a parallel Track B (expert PDF upload wiring) used the same days. Building a custom marketing surface from scratch under a ~6-hour total-dev budget was not compatible with also running the expert onboarding work in parallel.
**Decision**: Replicate the Framer "EvoTrack" template (AI/Smart-Tech landing) section-by-section: its dark-photo-hero + chartreuse-accent aesthetic, generous whitespace, accordion + card-grid + rounded-CTA-block primitives. Everything visual is copied; 100% of text is replaced with Spelix content from the landing-page copy plan. Ship **V1 = 6 sections** (Hero, Problem, HowItWorks, Differentiators, Privacy, FinalCta) by Day 2; **V2 = Four Dimensions + Roadmap** deferred to Sprint BETA (May 4-14). Design tokens (chartreuse `#d5ff45`, Host Grotesk + DM Sans, container 1128px) committed as Tailwind 4 `@theme` in `frontend/src/index.css`. Hero-bg photo deferred to V2 polish — current hero uses a subtle chartreuse radial-gradient on `surface-dark` so the component works without an external WebP. ResultsPage screenshot captured via Playwright MCP against live prod.
...
The original roadmap (internal strategy v1, 2026-04-11) deferred Phase 3. The revised plan (v3, 2026-04-14) pulled Phase 3 forward to a compressed 19-day sprint ending 2026-05-03, driven by internship application and interview timelines. The ARQ→streaq migration (Day 3-9) shipped on Day 4 via PR #48 + hotfix #49, freeing a 5-day buffer (Apr 16–22).

### Decision

Use the buffer to pull Phase 3 Batch 1 (P3-001/002/003) forward from the scheduled Day 10-13 window into Day 5-9. If Batch 1 ships cleanly by Day 9, Batches 2 + 3 proceed on the original cadence with additional buffer. If Batch 1 slips past Day 9, Batch 2 absorbs the remaining buffer and Batch 3 triggers the planned stop-loss (re-scope to drop distillation, retain agent core + sidebar).
...
**Related:** `backend/scripts/oneoff/e2e_fr_expv_08_test_accounts.py`. Previous practice documented in session 59 handoff §2 ("`app_metadata.role=admin + biomechanics_qualified=true` set on the maintainer admin account via Supabase admin API"). Memory `feedback_rate_limit_testing` ("Use separate test accounts for E2E") — ADR-E2E-01 now codifies the "how".

## ADR-DEPLOY-01: Deploy-script control-flow changes require real-deploy validation — inspection + CI cannot exercise post-`up -d` timing

**Context:** #303 (PR #305, commit `fbac6a6`) added an inner `if docker compose exec -T backend uv run alembic upgrade head; then <health loop> else echo "Migration failed — ROLLING BACK" fi` migrate-rollback gate to the `deploy` job's "Deploy via SSH" `script:` in `.github/workflows/ci.yml`. It passed the full T2 review chain (spec → quality → security, all PASS), `/code-review` (0 findings), CI green, and a human approval gate — **all by inspection**. Its OWN production deploy then **false-rolled-back on the happy path**: the gated `alembic upgrade` runs immediately after `docker compose up -d --build`, before the freshly-recreated backend is ready, so the post-`up -d` readiness race made the gate exit non-zero → rollback to the prior SHA (prod stayed healthy; rollback worked). Reverted via PR #306 (`7de82b0`). The bare `alembic upgrade head` line that preceded it "worked" precisely because the appleboy/ssh-action remote script runs **without `set -e`** — a transient failure was ignored and the 6×10s health-retry loop absorbed it; converting that command into a gate removed the tolerance.

**Decision:** Any change that adds or modifies a **failure-gate or control-flow branch** in the deploy `script:` (`.github/workflows/ci.yml`, "Deploy via SSH") MUST be validated against a **real deploy** (a forced/canary deploy that actually runs the script on the droplet) before being relied on — inspection, YAML-parse, and the CI lint/test/secret-scan suite cannot exercise post-`up -d` container-readiness timing. Corollary: **gating a backend-readiness-dependent command (migrate / `docker compose exec` / DB-connect) requires a readiness-retry** (retry with sleeps, or run only after `/health` passes), never a bare gate immediately after `up -d`. The deploy script's `set -e`-off semantics are load-bearing: bare commands tolerate transients, gates do not.

**Consequences:** Deploy-script `script:`-body changes are now a known high-risk class even when the diff is tiny and all gates PASS — the existing T2 governance covers the *paths*, this ADR adds the *validation method*. Future deploy-script gate work (e.g. the #303 redesign) must include a real-deploy validation step in its plan and treat "PASS on inspection" as insufficient. Reviewers (spelix-quality / spelix-spec) carry this in memory: flag any new gate on a readiness-dependent command. Cost: a real-deploy validation is slower and may itself roll back (safely) during iteration — acceptable vs. shipping a broken deploy pipeline. Re-validate this ADR if the deploy moves off appleboy/ssh-action or adopts `set -e`.

**Related:** PRs #305 (broke) / #306 (revert) / #307 (incident docs); issue #303 (reopened `needs-design`). `.github/workflows/ci.yml` deploy job. Reviewer memory: `spelix-quality-reviewer/review_deploy_migrate_rollback_303.md`, `spelix-security-reviewer/issue_303_migrate_rollback.md`. Builds on the deploy-CI false-negative posture (memory `insight-deploy-ci-false-negative`, #211) and ADR-GOVERNANCE-01 (T2 for CI deploy steps).
