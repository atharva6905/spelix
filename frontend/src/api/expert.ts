...
import { ApiError, buildApiError, isApiError } from "@/api/errors";
...
// ---------------------------------------------------------------------------
// Typed transport error (issue #235 → promoted to shared `@/api/errors`, #283)
// ---------------------------------------------------------------------------

// The typed transport error introduced for the expert surface in #235 is now
// the shared {@link ApiError}/{@link isApiError}/{@link buildApiError} in
// `@/api/errors`, used by every `@/api/*` module. These aliases preserve the
// historical `ExpertApiError`/`isExpertApiError` import surface for existing
// consumers; new code should import the shared names directly.
export { ApiError, isApiError, buildApiError } from "@/api/errors";
export const ExpertApiError = ApiError;
export type ExpertApiError = ApiError;
export const isExpertApiError = isApiError;
