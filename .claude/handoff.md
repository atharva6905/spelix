...
**R5 — expert-portal confidence + interpolation surfacing (shipped 2026-05-24, built via `/team`):** the Unvalidated Metrics panel now distinguishes "Not yet computed" (metric unimplemented) from "Cannot compute" (computed_yet but `null` this rep) and explains the latter with the rep's **categorical** landmark-confidence label (FR-CVPL-25 — never the raw decimal) + a new per-rep **interpolation fraction** ("{N}% interpolated"). New pure `compute_invalid_frame_mask` (`signal_processing.py`) reuses R2's exact gate predicate; pipeline Step 4 computes the clip mask once and Step 8 stores per-rep `mean(mask[start:end+1])` on a new nullable `RepMetric.interpolation_fraction` column (migration `47c8e446162e`, applied); expert service serializes it (no schema/endpoint change). Chose Option B (reuse confidence + one new signal) over a 40-site None-reason taxonomy — see ADR-R5-CONFIDENCE-INTERP-SURFACING. Real-fixture proof: `atharva-squat.mov` per-rep fractions `[0.31,0.08,0.24,0.62,0.04,0.22]` (rep 4 = 62% reconstructed — the deep-squat occlusion), `atharva-bench.mov` all `0.0` (not R2-gated). Verified: **2301 backend unit pass / 0 fail**, 2/2 R5 real-fixture integration, ruff+pyright clean; **765 frontend pass**, tsc clean; spelix-auditor **0 CRITICAL**, spelix-security-reviewer **PASS** (M-01 comment + M-02 zero-fraction guard fixed). Built by spelix-cv-engineer (T1 `c5b355e`) + spelix-migration (T2 `5b8bedd`) + spelix-tdd frontend (T4 `dc5b5b8`) + lead T3 `0506025`. **PR #176 merged (`f5c409f`); CI green incl. ✓ Deploy to Production; droplet HEAD `f5c409f`, containers healthy.** Prod E2E (expert portal, e2e-expert account): on a real squat analysis the panel shows per-rep "Very Low" confidence chips + "Cannot compute · Very Low confidence" cells (Lumbar Flexion Proxy, Technique Consistency) with the R2 tooltip; `GET /api/v1/expert/analyses/{id}` → **200** (confirms the `interpolation_fraction` column read-path is live on prod), 0 console errors. The "{N}% interpolated" line only appears on analyses processed AFTER this deploy (old reps have `interpolation_fraction=NULL`); fresh-upload demo deferred (same Storage-507/account constraint as R1/R2/R3/R6) — interp% is integration-proven. Backlog `L2-CV-DEPTHFRAME-R5` done.
...
- Fresh-upload prod demonstration of R1/R2/R3/R5/R6 deferred (account-identity + Supabase free-tier Storage 507 constraints; the corrected behaviour is integration-proven on the identical prod fixtures — R2 squat `hip_angle` [25.63,180.00], R5 squat interp fractions incl. rep 4 = 0.62, R6 rep 0 = 69.18°).
