...
## #235 extension (PR for issue #235, commit a4fb494, 2026-06-13) — typed ExpertApiError; the "plain object" premise above is now SUPERSEDED

(Persisted by the main agent on 2026-06-13 — the reviewer ran read-only in the T2 gate and asked for this to be recorded; Write tool was unavailable in its context.)

The #236 analysis above (line 16) hinged on "`expertFetch` throws a **plain object**, NOT an `Error`, so the `err instanceof Error` fallback never fires for HTTP errors." **#235 changes this:** `expertFetch` now throws a real `ExpertApiError extends Error` (`buildExpertApiError(status, body)` in `expert.ts`). So `err instanceof Error` / `isExpertApiError(err)` are now TRUE for every HTTP error, and the page's generic branch renders `(err as Error).message` for ALL non-DOI errors. **This remains PASS** — but the guarantee now rests on a different invariant:

- `ExpertApiError.message` is unwrapped ONLY from controlled sources: FastAPI `{detail:{error:{code,message}}}` → curated literal; Pydantic 422 array → first `{msg}` (schema/type-constraint text, no secrets); string `detail`; else the synthesized static fallback `Request failed (HTTP <n>).`. It is NEVER built from `str(exc)`, DB/constraint text, stack, or interpolated internals.
- Verified the actual raise-sites for INVALID_FILENAME / INVALID_PDF / QUEUE_UNAVAILABLE / INVALID_STATE / DUPLICATE_DOI / INVALID_DOI in `backend/app/api/v1/expert.py` (+ `app/utils/pdf_upload.py`, `app/utils/doi.py`): all fixed English literals. `INVALID_STATE` interpolates only the `review_status` enum; `DUPLICATE_DOI` interpolates another paper's `title` (corpus data the expert may view) and routes to the `doiError` FieldError, not the generic banner.

**Updated re-flag trigger (supersedes the #236 trigger):** because the page now renders `Error.message` generally, re-narrow / re-review if (a) any expert endpoint's HTTPException `message` starts interpolating `str(exc)` / DB text / filesystem paths / secrets, OR (b) a new Pydantic validator on these endpoints echoes raw user input into `msg`, OR (c) a new `expert.ts` throw wraps a caught exception body into `ExpertApiError.message`. XSS not a concern — rendered as React text content (`<div role="alert">{uploadError}</div>`, `FieldError` `<p>{children}`), default-escaped, no `dangerouslySetInnerHTML`.
