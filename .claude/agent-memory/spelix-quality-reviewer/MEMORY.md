...
## Reviewed: issue #311 / fix prod migrate command (COPY alembic + uv run --no-dev) (2026-06-15) → PASS (0 findings) — VALIDATED LIVE
- [Deploy migrate command fix #311](review_deploy_migrate_command_311.md) — root cause: bare `uv run` synced dev deps into the read-only `--no-dev` venv AND alembic.ini+alembic/ were never COPYd into the image. Fix bakes them in + `uv run --no-dev alembic -c /app/alembic.ini` on the #303 retry-then-gate; string-assertion tests are the right CI tool (real deploy can't run in CI). Verified `alembic` is a main dep, `%(here)s` resolves cwd-independently. **The #303 gate that exposed this was correct; #311 fixes the latent command. Real Deploy to Production ran GREEN — droplet `alembic current`=`9fffb59ba45f (head)`. ADR-DEPLOY-01 closed.**
