...
- [Issue #303 review](review_issue_303.md) --- #303 attempt 1 (#305) broke prod (backend-readiness race); redesign Option 1: 5x retry loop + MIGRATED gate + follow-up `[ "$m" = 5 ] || sleep 10` skip-sleep-on-last (human-approved): PASS, 2026-06-15
- [Issue #311 review](review_issue_311.md) --- prod migrate NEVER ran (missing `--no-dev` + alembic.ini/alembic never COPYd into image); fix bakes config+migrations in + `uv run --no-dev alembic -c /app/alembic.ini` on the #303 retry-gate; +exit-1 fail-red test (669228a): PASS, VALIDATED LIVE (deploy green, droplet alembic head), 2026-06-15
- [Issue #206 review](review_issue_206.md) --- bar-path trajectory on ResultsPage (FR-RESL-05, 3 reqs): PASS after fix (3e8a0be), deadlift-with-data now tested in both BarPathChart + ResultsPage, 2026-06-15
- **Recharts LineChart 2D-path pattern:** `type="number"` on `<XAxis dataKey="x">` makes each point plot at its true X coordinate — this IS a 2D spatial path chart, not a 1D series. Do not flag as degradation.
